...
- `letter_label.html` — per word: the real crop + CRAFT's cuts as **draggable polylines** (drag a
  ...
  reframe a mis-captured word; <kbd>⌘</kbd>+scroll / pinch **zooms** toward the cursor. Fix a
  **mis-recognized word** inline in the `text` field (e.g. load→had; labels + cut-count update live).
  **One loader** for everything (fresh `label_review.json` or a self-contained export, from any
  folder — the page is embedded); progress **auto-saves** to the browser. Exports a self-contained
