...
def vectorize_crop_inksight(crop: Image.Image) -> list[list[float]]:
    """Derender a single word crop into ``[[x, y, pen], ...]`` points (normalized [0,1])."""
...
def vectorize_boxes_inksight(
    ...
    word. Returns ``boxes`` (same schema as ``vectorize.vectorize_boxes``).
    """
    ...
    for entry in boxes if limit is None else boxes[:limit]:
        ...
        points = vectorize_crop_inksight(crop)
        entry["points"] = points
        entry["metadata"] = {
            "author": "robot",
...
def main(argv: list[str] | None = None) -> None:
    ...
    with open(boxes_path) as f:
        boxes = json.load(f)
    vectorize_boxes_inksight(args.pdf, boxes, args.page - 1, dpi=args.dpi, limit=args.limit)
