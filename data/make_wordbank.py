...
def generate_word_bank(args):
    ...
    for _ in range(args.num_examples):
        if args.add_digits and np.random.random() < args.digit_word_prob:
            ...
            word = (
                word.upper()
                if np.random.random() < args.uppercase_prob
                else (word.capitalize() if np.random.random() < args.capitalize_prob else word)
            )
        ...
        if np.random.random() < args.parenthesis_prob:
            word = "(" + word if np.random.random() < 0.5 else word + ")"

        # Add sentence flow punctuation at the end of the word
        if np.random.random() < args.sentence_flow_prob:
            for punctuation, prob in zip(args.punctuations, args.punctuation_probs, strict=False):
...
def analyze_word_bank(word_bank, k=75):
    ...
    for char, prob in sorted_probs:
        formatted_prob = f"{char!r:<{max_char_width}} : {prob:.2%}"
    ...
    for char, count in sorted(char_counts.items(), key=lambda x: x[1], reverse=True):
        formatted_count = f"{char!r:<{max_char_width}} : {count:>{max_count_width}}"
...
def main():
    ...
    parser.add_argument(
        "--num_examples", type=int, default=2000, help="Number of words to generate"
    )
    ...
    parser.add_argument(
        "--uppercase_prob", type=float, default=0.25, help="Probability of uppercase words"
    )
    parser.add_argument(
        "--capitalize_prob",
        type=float,
        default=0.25,
        help="Probability of capitalizing first letter",
    )
    parser.add_argument(
        "--length_slope", type=float, default=0.85, help="Slope for length probability distribution"
    )
    parser.add_argument("--output", default="bigbank.txt", help="Output filename")
    parser.add_argument("--seed", type=int, default=1337, help="Random seed for reproducibility")
    parser.add_argument(
        "--add_digits", action="store_true", default=True, help="Add digit words to the word bank"
    )
    parser.add_argument(
        "--digit_max_length", type=int, default=5, help="Maximum length for digit words"
    )
    parser.add_argument(
        "--digit_decimal_prob",
        type=float,
        default=0.3,
        help="Probability of adding a decimal point to digit words",
    )
    parser.add_argument(
        "--digit_word_prob", type=float, default=0.25, help="Probability of generating a digit word"
    )
    parser.add_argument(
        "--quote_prob",
        type=float,
        default=0.1,
        help="Probability of adding a quote (single or double) to a word",
    )
    parser.add_argument(
        "--parenthesis_prob",
        type=float,
        default=0.1,
        help="Probability of adding a parenthesis (open or close) to a word",
    )
    parser.add_argument(
        "--sentence_flow_prob",
        type=float,
        default=0.80,
        help="Probability of adding sentence flow punctuation to a word",
    )
    parser.add_argument(
        "--punctuations",
        nargs="+",
        default=[",", ".", "?", "!"],
        help="List of punctuations to use",
    )
    parser.add_argument(
        "--punctuation_probs",
        nargs="+",
        ...
        help="Probabilities for each punctuation",
    )
    ...
    # letter_probs = np.array([6, 4, 4, 4.3, 6, 4, 4, 6.1, 6, 4, 4, 4, 4,
    #                          6, 6, 4, 4, 6, 6.3, 6, 4, 4, 4, 4, 4, 4])
